<template>
    ...
    <header class="header">
      <h1>todos</h1>
      <input v-model="newContent" @keyup.enter="add" class="new-todo" placeholder="What needs to be done?"  autofocus>
    </header>
    ...
    <section class="main">
      ...
      <ul class="todo-list">
        <li v-for="(item,i) in currentList" :key="i" :class="[item.checked ? 'completed':'', item.isEdit ? 'editing':'']" >
          <div class="view">
            <input class="toggle" type="checkbox" v-model="item.checked">
            <label v-on:dblclick="edit(i)">{{item.content}}</label>
            <button class="destroy" @click="del(i)"></button>
          </div>
          <input class="edit" v-model="item.content" @keyup.enter="endEdit(i)">
        </li>
      </ul>
    </section>
    <footer class="footer">
      ...
      <ul class="filters">
        ...
        <li>
          <a :class="filter === 1? 'selected':''" @click="filterActive">Active</a>
        </li>
        <li>
          <a :class="filter === 2? 'selected':''" @click="filterCompleted">Completed</a>
        </li>
      </ul>
  ...
  computed: {
    currentList: function () {
      if (this.filter === 0) {
        ...
        return this.list.filter(ele => ele.checked === false)
      } else {
